...
const ListedWishList = ({ sortingType }) => {
  ...
  const sortedWishlist = useMemo(() => {
    ...
    const sorted = [...wishlist]; // Create a copy to avoid mutating original
      ...
      <div className="mb-8 flex justify-between items-center flex-wrap gap-4">
        <div>
          ...
          <p className="text-gray-500">
            You have {wishlist.length} book{wishlist.length !== 1 ? "s" : ""} in
            your wishlist
            {sortingType && (
        ...
        <button
          onClick={() => {
            setWishlist([]);
            toast.info("All books removed from wishlist");
          }}
